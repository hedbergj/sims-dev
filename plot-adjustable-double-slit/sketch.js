...
function calcFunction() {
  ...
  for (var x = 0; x < y.length; x += 1) {
    theta = map(x,0,y.length,0,PI/2)
    y[x] = amplitude * Math.pow(Math.cos(PI*d*Math.sin(theta)/lambda),2)*Math.pow(Math.sin(PI*b*Math.sin(theta)/lambda)/(PI*b*Math.sin(theta)/lambda),2)
